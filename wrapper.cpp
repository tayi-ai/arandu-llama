...
// Not static: wrapper_adapter.cpp writes the same slot, so a caller reads one
// error whichever half produced it. Internal linkage here would give that file
// its own, and two error slots is two places to look when something fails.

...
ifeq ($(BUILD_TYPE),metal)
	...
	# No EXTRA_TARGETS here, unlike the other backends. Metal used to compile to
	# a single ggml-metal.m.o that got folded into libbinding.a; upstream split
	# it into a backend library of its own, and the copy failed with
	#
	#   cp: CMakeFiles/ggml.dir/ggml-metal.m.o: No such file or directory
	#
	# after every object in llama.cpp had already been compiled. Nothing is lost
	# by dropping it: libggml-metal.a is in BACKEND_STATIC_LIBS below and
	# -lggml-metal is in the darwin LDFLAGS, so the objects are linked either
	# way.
...
# The ggml-metal.o rule is gone with the target that asked for it. Metal is a
# backend library now, not one object; see the BUILD_TYPE=metal block above.

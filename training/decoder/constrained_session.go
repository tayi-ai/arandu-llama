...
// ConstrainedSessionRecipe pins a concrete native implementation of one fusion
// stage whose checkpoint, teacher caches and projections already exist.
// Initial restores the externally admitted adapter, not optimizer moments.
...
func (c ConstrainedFactoryConfig) validate() error {
	...
	if p == nil || c.Shards == nil || r.MaxConfigBytes < 1 || r.MaxConfigBytes > 64<<20 ||
		...
		r.Initializer.ExpectedSHA256 != p.summary.Identity.InitialAdapterSHA256 || r.Initial.ParametersSHA256 != r.Protocol.InitialParametersSHA256 ||
		!validAssemblyHash(r.Initial.FileSHA256) || r.Initial.Path == "" || r.Initial.MaxBytes < 1 || r.Initial.MaxWorkingBytes < 1 ||
		...
		len(r.Signals) != len(r.Protocol.Updates) || len(r.Signals) < 1 || len(p.adapters) != len(r.Protocol.Layout) {
	...
	if stage.ID == "" || stage.MaxSteps != len(r.Signals) || int64(stage.MaxTokens) > r.Limits.MaxTokens || stage.Phase != pipeline.PhaseFusion {
		return ErrConstrainedSession
	}
	...
	for i, signals := range r.Signals {
		...
		for _, pair := range r.Protocol.Updates[i].Config.Protection {
...
func (c ConstrainedFactoryConfig) memory() error {
	...
	for _, signals := range r.Signals {
		signalBytes = max(signalBytes, signals.Limits.MaxBytes)
	}
...
// Only PhaseFusion is supported. Recovery requires a separate ground-truth
// objective; it is not silently relabeled as teacher/feature distillation here.
...
func newConstrainedFactory(config ConstrainedFactoryConfig, load constrainedAssemblyLoader) (pipeline.ConstrainedFactory, error) {
	...
	return func(ctx context.Context, request pipeline.ConstrainedRequest) (result pipeline.ConstrainedSession, err error) {
		...
		if request.Step == 1 {
			_, err = loaded.RestoreAdapter(ctx, config.Recipe.Initial)
		} else {
...
func (c ConstrainedFactoryConfig) request(request pipeline.ConstrainedRequest) error {
	r := c.Recipe
	if request.Step < 1 || request.Step > len(r.Signals) || request.Context.Execution.TargetSHA256 != r.Recipe.Student.SHA256 || !reflect.DeepEqual(request.Context.Execution.Recipe, r.Recipe) || !reflect.DeepEqual(request.Context.Execution.Placement, r.Placement) ||
...
func (c ConstrainedFactoryConfig) signals(ctx context.Context, request pipeline.ConstrainedRequest) (*pipeline.Signals, error) {
	for _, source := range request.Sources {
		if err := verifyConstrainedSource(ctx, source); err != nil {
			return nil, err
		}
	}
	...
	for _, cache := range spec.Caches {
		...
		sources = append(sources, pipeline.CacheInput{Path: source.Path, SHA256: source.Source.Artifact.SHA256, Bytes: source.Source.Artifact.Bytes, Expectation: cache.Expectation, Weight: cache.Weight, Projections: cache.Projections})
	}

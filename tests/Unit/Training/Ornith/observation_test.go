...
func TestObservedForwardPreservesValuesAndReportsEveryPlacement(t *testing.T) {
	for _, dtype := range []torch.DType{torch.Float16, torch.Float32} {
		t.Run(map[torch.DType]string{torch.Float16: "fp16", torch.Float32: "fp32"}[dtype], func(t *testing.T) {
			...
			var observations []ornith.ForwardObservation
			observed, err := f.model.ForwardObserved(context.Background(), f.tokens, f.limits, func(_ context.Context, item ornith.ForwardObservation) error {
				copy := item
			...
			for _, item := range observations {
				if item.Device != torch.CPUDevice() || item.Shape[0] != 1 || !item.AllFinite || item.NonfiniteElements != 0 || item.NonzeroElements == 0 || item.Minimum == nil || item.Maximum == nil || *item.Minimum > *item.Maximum || item.ChunkBytes <= 0 || item.ChunkBytes > ornith.ObservationChunkBytes {
			...
			if observations[0].Stage != ornith.StageEmbedding || observations[0].Layer != -1 || observations[0].DType != dtype {
...
func TestObserverFailureStopsAndLeavesModelReusable(t *testing.T) {
	for _, stage := range []ornith.ForwardStage{ornith.StageEmbedding, ornith.StageBeforePlacement, ornith.StageAfterPlacement, ornith.StageAfterDecoder, ornith.StageFinalNorm, ornith.StageLogits} {
		t.Run(string(stage), func(t *testing.T) {
			...
			failed := false
			snapshot, err := f.model.ForwardObserved(context.Background(), f.tokens, f.limits, func(_ context.Context, item ornith.ForwardObservation) error {
				if failed {
...
func TestObservedForwardCancellationStopsBeforeNextStage(t *testing.T) {
	...
	calls := 0
	snapshot, err := f.model.ForwardObserved(ctx, f.tokens, f.limits, func(_ context.Context, item ornith.ForwardObservation) error {
		calls++
		if item.Stage == ornith.StageAfterPlacement {
			cancel()
		}
		return nil
	})
	...
	snapshot, err = f.model.ForwardObserved(ctx, f.tokens, f.limits, func(context.Context, ornith.ForwardObservation) error {
...
func TestObservationPreservesHalfBytesAndReportsNonfiniteValues(t *testing.T) {
	...
	calls := 0
	snapshot, err := f.model.ForwardObserved(context.Background(), []int64{0, 1}, f.limits, func(_ context.Context, item ornith.ForwardObservation) error {
		calls++
		digest := sha256.Sum256(body[:16])
		if item.Stage != ornith.StageEmbedding || item.AllFinite || item.NonfiniteElements != 2 || item.NonzeroElements != 4 || item.Minimum == nil || *item.Minimum != -2 || item.Maximum == nil || *item.Maximum != 65504 || item.SHA256 != hex.EncodeToString(digest[:]) {
...
func TestObservationStreamsMoreThanOneChunkWithoutConvertingBytes(t *testing.T) {
	...
	tokens := make([]int64, ornith.ObservationChunkBytes/(4*4)+2)
	for index := range tokens {
		tokens[index] = 1
	}
	limits := ornith.Limits{MaxTokens: int64(len(tokens)), LogitRows: 2, MaxCheckpointBytes: int64(len(tokens)) * 4 * 4 * 34}
	...
	snapshot, err := f.model.ForwardObserved(context.Background(), tokens, limits, func(_ context.Context, item ornith.ForwardObservation) error {
		if item.Stage != ornith.StageEmbedding || item.ChunkBytes != ornith.ObservationChunkBytes || item.Bytes != int64(len(tokens)*16) || item.SHA256 != hex.EncodeToString(digest.Sum(nil)) || !item.AllFinite || item.NonzeroElements != int64(len(tokens)*4) || item.Minimum == nil || math.IsNaN(*item.Minimum) {

...
//
// It is read from the model and never taken from the caller. The question this
// package's readings exist to answer is what a lower bit-width costs, and that
// question is a comparison with everything else held equal -- so a reading
// labelled with the wrong representation is worse than an unlabelled one,
// because it enters the comparison and moves the answer.
func (m *Model) Describe() (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	// 256 bytes holds any description llama.cpp produces; the longest observed is
	// under sixty.
	buffer := make([]C.char, 256)
	written := C.llama_wrapper_model_describe(m.modelPtr, &buffer[0], C.int(len(buffer)))
	runtime.KeepAlive(buffer)
	if written < 0 {
		return "", fmt.Errorf("failed to describe model: %s", C.GoString(C.llama_wrapper_last_error()))
	}
